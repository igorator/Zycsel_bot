...
const { upsertPost, upsertMessage } = require('../../model/channelPosts');

const setupChannelPostController = (bot) => {
  bot.on('channel_post:media', async (ctx) => {
    ...
    if (channelPostData.caption) {
      ...
      if (channelPostData.caption.match(SEX_REGEX)) {
        sex = channelPostData.caption
          .match(SEX_REGEX)[0]
          .replace('#стать_', '');
      }
...
module.exports = { setupChannelPostController };
